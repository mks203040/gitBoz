...
$albumsPerPage = '300';       // number of albums per page
$itemsPerPage  = '300';       // number of images per page    
$thumb_width   = '135';      // width of thumbnails
$thumb_height  = '200';       // height of thumbnails
...
if (!isset($_GET['album'])) {
   ...
   echo '<div class="titlebar">
           ...
           <div class="float-right">'.count($files).' изображения</div>
         </div>';	  
   echo '<div class="clear"></div>';
   ...
   for( $i=$start; $i<$start + $itemsPerPage; $i++ ) {
		  ...
		  if( isset($files[$i]) && is_file( $src_folder .'/'. $files[$i] ) ) { 
	        ...
	        echo '<table style="border=0;margin-left:-5px;padding:0;" class="thumb"><tr><td><div>
			      ...
			      </div></td></tr><tr><td style="text-align:center;"><p style="font-size:10px;font-weight:bold;color:blue; margin-left:10px;text-decoration:underline; ">Титул папки</p></td></tr></table>'; 
